...
// - 어떤 요청을 처리할지 주소매핑
// - 요청처리된 결과에 따라 응답하는 방법 제어
// (forward, redirect, 값 등)

// * instance : 개발자가 new를 이용해 만든객체
// * bean     : Spring(Java)이 만든 객체

// @Controller 어노테이션
// 1) Controller 임을 명시
// 2) 클래스에 작성된 내용대로 구현 
// 	 -> 스프링이 객체(Object, instance) == Bean 등록 / Bean 생성
...
public class ForwardTestController {
	  ...
	  // == 해당 클래스를 "요청주소"를 매핑해서 처리하는 클래스
		...
		/* * Spring Controller *
		 * 
		 * - @RequestMapping("요청주소") -> 클래스명, 메서드명 위에작성
		 *   @GetMapping("요청주소")	 	 -> 메서드명 위에작성
		 *   @PostMapping("요청주소")    -> 메서드명 위에작성
		 * 	 
		 * @RequestMapping("요청주소") 뜻
		 *	- 요청주소를 처리할 클래스/메서드를 매핑하는 어노테이션(컴파일러한테 말하는주소) 	
		 * 
		 * 1) 클래스 위에 작성하는 경우
		 *    - 공통주소를 해당 클래스에 매핑
	...
	// 5) 매핑할 요청주소를 @RequestMapping 등을 이용해 작성 

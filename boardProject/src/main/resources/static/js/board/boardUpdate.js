...
// * Set을 사용하는 이유 : X 버튼이 눌러질 때마다
// order가 저장될 예정인데 중복되는 값을 저장 못하게 하기 위해
...
const updatePreview = (file, order) => {
  ...
  reader.addEventListener("load", e => {
    ...
    // -> 왜?? 이전에 X버튼을 눌러 삭제기록이 있을 수 도 있기 때문에
...
for (let i = 0; i < inputImageList.length; i++) {
  ...
  deleteImageList[i].addEventListener("click", () => {
    ...
    // X 버튼이 눌러졌을때
    // --> 기존에 이미지가 있었는데 i번째 이미지 X버튼 눌러서 삭제함
    // --> DELETE 수행
...
form.addEventListener("submit", e => {
  ...
  // 제출전에 form 태그 마지막 자식으로
  // input을 추가 한 후에 제출
  // -> 해당 input에는 
  // 삭제된 이미지 순서(deleteOrderList)를 담음

  const input = document.createElement("input");

  // Array.form() : set -> Array로 변환

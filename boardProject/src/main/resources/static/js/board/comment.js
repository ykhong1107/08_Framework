...
const insertComment = (parentCommentNo) => {
  ...
  .then(commentNo => {
    ...
    if(parentCommentNo === undefined){
      const content
       = `<strong>${memberNickname}</strong>{닉네임}님이 <strong>${boardDetail.boardTitle}</strong>{게시글 제목} 게시글에 댓글을 작성했습니다`;
...
// ** 답글 작성 textarea가 한 개만 열릴 수 있도록 만들기 **
  ...
  // 클릭된 답글버튼이 속해있는 댓글(li) 요소찾기
  // closest("태그") : 부모중 가장 가까운 태그찾기
...
const showUpdateComment = (btn) => {
  ...
  /** 댓글 수정화면이 1개만 열려 있을 수 있게하기 */
  // == 이미 열려있는 수정화면이 있으면 닫아버리기
  ...
  // 2. 취소버튼 동작에 대비하여
  //    현재 댓글(commentRow)의 HTML 코드를 백업

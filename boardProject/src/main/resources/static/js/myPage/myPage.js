...
updateInfo?.addEventListener("submit", e => {
  ...
  // empty, notEmpty 중 3이 하나도 없을경우
  // if(empty !== 3 && notEmpty !== 3) 아래와 같음
...
// "변경하기" 버튼 클릭 시 또는 form 내부 엔터(enter) 입력 시
// == submit (제출)
changePw?.addEventListener("submit", e => {

  // e.preventDefault(); // 기본이벤트 막기
  // -> form의 기본 이벤트인 제출막기
  // -> 유효성 검사조건이 만족되지 않을 때 수행
  ...
  */
  ...
  if ( !(lengthCheck && letterCheck && numberCheck && specialCharCheck) ) {
      alert("영어, 숫자, 특수문자 1글자이상, 6~20자 사이로 입력해주세요")
      e.preventDefault();
      return;
  }
...
// 프로필 이미지업로드 상태에 따라서 어떤 상태인지 구분하는 값
...
let lastVaildFile = null; // 마지막으로 선택한 파일을 저장할 변수
  ...
  */
  const updatePreview = (file) => {
    ...
    reader.addEventListener("load", e => {
      ...
      statusCheck = 1; // 새 파일이 선택됨 상태체크
    })


  }

  /** input type="file" 태그가 선택한 값이 변한경우 수행 */
  imageInput.addEventListener("change", e =>{
    ...
    if(file === undefined){
      ...
      /** 이전 선택한 파일 유지하는 코드 */
      // -> 이전 선택한 파일을 저장할 전역변수(lastVaildFile) 선언
      
      // 이전에 선택한 파일이 없는경우
      // == 현재 페이지 들어와서 프로필 이미지 바꾼적이 없는경우
      if(lastVaildFile === null) return;

      // 이전에 선택한 파일이 "있을" 경우
      const dataTransfer = new DataTransfer();

      // dataTransfer 객체가 가지고있는 files 필드에
      // lastVaildFile 추가

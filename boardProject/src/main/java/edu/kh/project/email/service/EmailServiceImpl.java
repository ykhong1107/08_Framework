...
public class EmailServiceImpl implements EmailService {
	...
	public int sendEmail(String htmlName, String email) {
		...
		try {
			...
			// 매개변수 1 : MimeMessage
			// 매개변수 2 : 이메일에 파일첨부 여부 // true -> 첨부한다 false -> 첨부안한다
			...
			// 매개변수 1 : 이메일 내용
			// 매개변수 2 : HTML 코드 해석여부 지정(true == 해석 O)
			
			// 지정된 HTML 파일에 authKey가 첨부된 후
			// HTML 코드전체가 하나의 String으로 변환되서 반환 받은 후
			// 반환된 String을 메일 내용으로 세팅
			
			// CID(Content-ID)를 이용해 메일에 이미지 첨부
			helper.addInline("logo", 
							new ClassPathResource("static/images/logo.avif"));

...
//@Aspect // 공통관심사(advice)가 작성된 클래스임을 명시(AOP 동작에 참여)
@Slf4j
public class TestAspect {
	...
	// Poingcut :
	// 	후보 지점 중 실제 적용될 지점을 잘라낸 것(실제로 적용되고 수행할 지점)
	...
	/**
	 * 모든 Controller 메서드 수행 전에(Before) 로그 출력
	 */
...
//	@After("execution(* edu.kh.project..*Controller*.*(..))")
	@After("PointcutBundle.serviceImplPointcut()")
	public void controllerEnd(JoinPoint jp) {
		// JoinPoint : AOP에서 사용가능한 부가 기능을 가진 객체

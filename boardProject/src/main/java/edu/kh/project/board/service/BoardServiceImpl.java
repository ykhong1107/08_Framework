...
public class BoardServiceImpl implements BoardService{
	...
	public Map<String, Object> selectBoardList(int boardCode, int cp) {
		...
		// 1. boardCode가 일치하는 게시글의 전체 개수조회
		//   (조건 : 삭제되지 않은 글만 카운트!)
		...
		// 3. DB에서 cp(조회하려는 페이지)에 해당하는 행을 조회하는코드
		// ex) cp == 1,  전체목록 중  1~10행 결과만 반환
		//     cp == 2,  전체목록 중 11~20행 결과만 반환
		//     cp == 10, 전체목록 중 91~100행 결과만 반환
		...
		// 4. 목록조회 결과 + Pagenation 객체를 Map으로 묶어서 반환
